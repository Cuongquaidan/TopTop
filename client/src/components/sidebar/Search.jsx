...
const Search = ({ option, setOption }) => {
  ...
  const renderStatusIcon = (status) => {
    ...
    return <GoClockFill className='text-gray-400' />;
  };

  return (
    <div className='w-[400px] bg-white  rounded-xl shadow-xl  h-screen flex flex-col'>
      <div className="flex justify-between items-center px-6 mb-4 ">
        <p className="text-xl font-bold">Tìm kiếm</p>
        <button onClick={() => setOption('')} className="text-neutral-300">
          <MdCancel size={24} />
        </button>
      </div>
      ...
      <div className=" pb-6  h-full  flex flex-col">
        <input
          ...
          className='outline-none text-slate-950 px-4 focus:border-primary border-slate-300 p-3 border rounded-full w-[calc(100%-40px)] mx-auto mb-4'
        />

       <div className='grow overflow-y-scroll pb-10 px-5 '>
       <div className="mb-6">
          <p className='font-semibold mb-2 text-slate-600'>Tìm kiếm gần đây</p>
          <ul>
            {(showMore ? recentSearches : recentSearches.slice(0, 5)).map((item, index) => (
              <li key={index} className='flex items-center justify-between py-1 text-md text-slate-700 hover:underline cursor-pointer'>
                <div className='flex items-center gap-4'>
                  <GoClockFill className='text-gray-400' />
                  <span>{item}</span>
                </div>
          ...
          <button
            ...
            className='text-blue-500 text-md mt-1 hover:underline'>
            {showMore ? 'Ẩn bớt' : 'Xem thêm'}
          </button>
        </div>

        <div className="">
          <p className='font-semibold mb-2 text-slate-600'>Bạn có thể thích</p>
          <ul className='space-y-2 pb-10'>
            {suggestedSearches.slice(0,8).map((item, index) => (
              <li key={index} className='flex items-center gap-4 text-md text-slate-800 hover:underline cursor-pointer'>
