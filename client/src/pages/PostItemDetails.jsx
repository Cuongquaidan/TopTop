    ...
    return (
      <div className="w-screen h-screen grid grid-cols-[70vw_30vw] relative overflow-hidden">
        <div className="relative w-full h-full">
          <div className="fixed top-10 left-[35vw] z-[999] transform -translate-x-1/2 backdrop-blur-2xl flex justify-center" style={{ width: width }}>
            <input type="text" placeholder="Tìm kiếm ..." className="outline-none border-[1px] p-4 rounded-4xl w-[90%] text-neutral-50 border-neutral-50" />
          ...
          <div className="fixed top-10 left-10 z-[999]">
            <MdCancel size={60} className="text-white/80 cursor-pointer" onClick={goBack} />
          </div>

          <div className="absolute inset-0 z-0" style={{
            backgroundImage: currentPost?.type === "video"
        ...
        {currentPost && (
          <div className="relative z-20 flex flex-col p-4 gap-4 text-black dark:text-gray-200 w-[544px] bg-white dark:bg-neutral-900 rounded-lg">
            <div className="flex justify-between items-start">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <img src={currentPost.user.profile_picture} alt={currentPost.user.display_name} className="w-14 h-14 rounded-full object-cover border-2 border-pink-500" />
                  <div>
                    <div className="flex items-center gap-1">
                      <span className="font-bold text-lg dark:text-white">{currentPost.user.display_name}</span>
                      {currentPost.user.username === "hnh5072" && <FiAlertTriangle className="text-yellow-400" />}
                    </div>
                    <p className="text-sm text-neutral-700 dark:text-gray-400">@{currentPost.user.username}</p>
                  </div>
                </div>
              </div>
              <button className="bg-primary text-white font-semibold px-4 py-1 rounded-lg">Follow</button>
            </div>

            {currentPost.caption && (
              <p className="text-lg text-neutral-900 dark:text-gray-300">{currentPost.caption}</p>
            )}

            <div className="flex justify-between items-center text-sm">
              <div className="flex gap-4 text-neutral-900">
                <div className="flex items-center gap-2 cursor-pointer" onClick={() => handleLikePost()}>
                  <div className="flex items-center justify-center rounded-full w-8 h-8 bg-neutral-200 dark:bg-neutral-700">
                    <FaHeart size={16} className={isLiked ? "text-red-600" : "dark:text-gray-300"} />
                  </div>
                  <p className="text-[12px] font-bold text-neutral-900 dark:text-gray-300">{convertNumToString(numOfLikes)}</p>
                </div>

                <div className="flex items-center gap-2 cursor-pointer">
                  <div className="flex items-center justify-center rounded-full w-8 h-8 bg-neutral-200 dark:bg-neutral-700">
                    <BiSolidMessageRoundedDots size={16} className="dark:text-gray-300" />
                  </div>
                  <p className="text-[12px] font-bold text-neutral-900 dark:text-gray-300">{convertNumToString(currentPost.numOfComments)}</p>
                </div>

                <div className="flex items-center gap-2 cursor-pointer" onClick={() => handleSavePost()}>
                  <div className="flex items-center justify-center rounded-full w-8 h-8 bg-neutral-200 dark:bg-neutral-700">
                    <IoBookmark size={16} className={isSaved ? "text-orange-400" : "dark:text-gray-300"} />
                  </div>
                  <p className="text-[12px] font-bold text-neutral-900 dark:text-gray-300">{convertNumToString(numOfSave)}</p>
                </div>
              </div>
